...
class Audit extends Base {
	...
	public function rest_results_get( $rest_post, $field_name, $request ) {
		...
		foreach ( $standards as $standard ) {
			...
			/*
			 * This will become irrelevant once scores are no longer generated by the Audit Server and stored by the API,
			 * but since existing data may still include historical scores, we need to prevent them from surfacing in the results
			 */
			if ( isset( $results[ $standard ]['scores'] ) ) {
				unset( $results[ $standard ]['scores'] );
			}
